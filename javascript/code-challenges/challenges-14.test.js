...
const screenForNames = (arr) => {
  ...
  return arr.filter((z) => reg.test(z));
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map((z) => z.charAt(0).toUpperCase() + z.slice(1));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(z => Number(z.height) > Number(arr[0].height))
    .map(z => z.name)
    .join(' - ');
};
...
const sortBy = (property, arr) => {
  ...
  return arr.sort((x1, x2) => {
    return x1[property] > x2[property] ? 1 : -1;
  });
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let r1 = board[0];
  let c1 = board.map((e) => e[0]);
  let r2 = board[1];
  let c2 = board.map((e) => e[1]);
  let r3 = board[2];
  let c3 = board.map((e) => e[2]);
  let d1 = [board[0][0], board[1][1], board[2][2]];
  let d2 = [board[2][2], board[1][1], board[0][0]];
  let whole = [r1, r2, r3, c1, c2, c3, d1, d2];
  let truthArr = whole.map((z) => z.every((v) => v === z[0] && v));
  return !truthArr.every((boolean) => boolean === truthArr[0]);
};
