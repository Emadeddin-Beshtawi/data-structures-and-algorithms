...
// Solution code here...
return arr.indexOf(arr.reduce((x1,x2)=> x1.length>x2.length?x1:x2,0))
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(z=> z.includes(':)'));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(z => z.split('').filter(y => /\d/g.test(y)).join(''));
};
