...
const sortByChildren = (charArray) => {
  ...
  return charArray.sort((x1, x2) => {
    if (x1.children.length === x2.children.length) {
      if (x1.house > x2.house) {
        return 1;
      } else {
        return 0;
      }
    } else {
      return x1.children.length - x2.children.length;
    }
  });
};
...
const containsW = (str) => {
  // Solution code here...
  const manner = /w/g;
  const regularW = manner.test(str);
  return regularW;
};
...
const isNum = (input) => {
  // Solution code here...
  const manner = /[0-9]/;
  const regularW = manner.test(input);
  return regularW;
};
...
const containsWorld = (input) => {
  ...
  const regularW = /world/g;

  if (regularW.test(input)) {
    return true;
  } else {
    return false;
  }
...
const isCapitalized = (str) => {
  // Solution code here...
  const manner = /[A-Z][a-z]*/g;
  const regularW = str.match(manner);
  return regularW || [];
};
