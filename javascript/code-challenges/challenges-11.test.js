...
const count = (target, input) => {
  // Solution code here...
  let initialValue = 0;
  input.map((z) => z.map((x) => x === target && initialValue++));
  return initialValue;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((e) =>
    e
      .filter((y) => typeof y === "number" && y % 5 === 0)
      .map((x) => Math.pow(2, x))
  );
};
...
let findMaleAndFemale = (data) => {
  ...
  return data
    .filter((j) => {
      return j.gender === "male" || j.gender === "female";
    })
    .map((j) => j.name)
    .join(" and ");
};
...
let findShortest = (data) => {
  ...
  return data.reduce((x, y) => {
    return Number(x.height) - Number(y.height) < 0 ? x : y;
  }, 0).name;
};
